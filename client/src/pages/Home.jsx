...
function Home() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:5000/api/art/generate', { title, prompt, style });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/art/save', { title, prompt, imageUrl: generatedImage, style, tags: tagArray });
  ...
  const handlePublish = async () => {
    ...
    try {
      const tagArray = tags.split(',').map(tag => tag.trim()).filter(tag => tag);
      await axios.post('http://localhost:5000/api/art/publish', { title, prompt, imageUrl: generatedImage, style, tags: tagArray });
