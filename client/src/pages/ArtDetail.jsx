...
function ArtDetail() {
  ...
  const fetchArtDetail = async () => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:5000/api/art/${id}`);
      setArt(data);
      setComments(data.comments || []);
    } catch (error) {
      // If specific endpoint doesn't exist, get from gallery and find the art
      try {
        const { data: galleryData } = await axios.get('http://localhost:5000/api/art/gallery');
  ...
  const handleComment = async (e) => {
    ...
    try {
      const { data } = await axios.post(`http://localhost:5000/api/art/${id}/comment`, {
  ...
  const handleDeleteComment = async () => {
    ...
    try {
      const { data } = await axios.delete(
        `http://localhost:5000/api/art/${id}/comment/${selectedComment._id}`
      );
      setComments(data.comments);
